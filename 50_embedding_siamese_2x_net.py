...
# 50_embedding_siamese_2x_net preprocceses the data-set into word embeddings and trains the siamese
...
MAX_NUM_WORDS= 50000
MAX_SEQUENCE_LENGTH = 50 # each title could be 100 tokens each.
GLOVE_PATH = '/home/pablo/FYP/glove.6B/glove.6B.100d.txt'
CPI_DATASET_DIR = '/home/pablo/FYP/1000_with_titles_siamese_2x_50_tokenized_results/'
...
if args.reload_embeddings:
    ...
    # add zeros to the vector to make it len 50
    p_a = pad_sequences(tokenizer.texts_to_sequences(data['tokenized_text_a'].map(str)), maxlen=MAX_SEQUENCE_LENGTH)
    p_b = pad_sequences(tokenizer.texts_to_sequences(data['tokenized_text_b'].map(str)), maxlen=MAX_SEQUENCE_LENGTH)
    texts_embeddings = pd.concat([pd.DataFrame(p_a), pd.DataFrame(p_b)], axis=1)
    texts_embeddings.columns = [x for x in range(100)]
...
a_columns = [str(x) for x in range(50)]
# Columns titled "200" to "399" are the embeddings for text_b
b_columns = [str(x) for x in range(50, 100)]
...
print(x_train[a_columns].head())
print(x_train[b_columns].head())
history = model.fit([x_train[a_columns], x_train[b_columns]], y_train, validation_split=0.1, epochs = 50)
